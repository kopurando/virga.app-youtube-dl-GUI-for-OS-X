...
* Downloads search results (e.g. search on youtube), __use with cauton!__ There is no "Stop/Cancel" button yet :) 
...
* Updates youtube-dl if media download fails
...
- Stop/cancel button to abort downloads in progress (e.g. create pid-files and kill processes on repeated virga launch with the same URL?)
